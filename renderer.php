...
/**
 * Outputs the assignments overview tree.
 *
...
/**
 * Renderer for Assignments overview block
 *
 * @package   block_assignments_overview
 * @copyright 2018 Tanmay Gatle Manasi Ladkat
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class block_assignments_overview_renderer extends plugin_renderer_base {
     ...
     * @param array $assignmentsubmissions
    ...
    public function assignments_tree($courses, $assignments, $assignmentgrades, $assignmentsubmissions) {
        ...
        $content = html_writer::start_tag('ul',
            array('id' => 'mycoursestree', 'class' => 'tree courses_ul', 'role' => 'tree'));
        ...
        foreach ($courses as $course) {
                ...
                if ($assignment->course === $course->id) {
                    ...
                    foreach ($assignmentgrades as $assignmentgrade) {
                        if ($assignmentgrade->assignment === $assignment->id) {
                            $submitstatus = html_writer::tag('li',
                                get_string("status", "block_assignments_overview") .
                                get_string("graded", "block_assignments_overview"),
                                array('role' => "treeitem", 'tabindex' => '-1'));
                            $grade = html_writer::tag('li',
                                get_string('gradedlabel', "block_assignments_overview") .
                                $assignmentgrade->grade, array('role' => "treeitem", 'tabindex' => '-1'));
                            $assignmentdetails = html_writer::tag('ul',
                                $submitstatus . $grade,
                                array('role' => 'group', 'class' => 'assignment_details_ul'));
                            $isgraded = true;
                        ...
                        foreach ($assignmentsubmissions as $assignmentsubmission) {
                            if ($assignmentsubmission->assignment === $assignment->id) {
                                if ($assignmentsubmission->status == "submitted") {
                                    $submitstatus = html_writer::tag('li',
                                        get_string("status", "block_assignments_overview") .
                                        get_string("submitted", "block_assignments_overview"),
                                        array('role' => 'treeitem', 'tabindex' => '-1'));
                                    $duedate = html_writer::tag('li',
                                        get_string("duedate", "block_assignments_overview") .
                                        '<br>' . userdate($assignment->duedate),
                                        array('role' => 'treeitem', 'tabindex' => '-1'));
                                    $assignmentsubmitteddate = html_writer::tag('li',
                                        get_string("assignmentsubmitteddate",
                                            "block_assignments_overview") . '<br>' .
                                        userdate($assignmentsubmission->timemodified),
                                        array('role' => 'treeitem', 'tabindex' => '-1'));
                                    $assignmentdetails = html_writer::tag('ul',
                                        $submitstatus . $assignmentsubmitteddate . $duedate,
                                        array('role' => 'group', 'class' => 'assignment_details_ul'));
                                    $issubmitted = true;
                    ...
                    if (!$isgraded && !$issubmitted) {
                        $submitstatus = html_writer::tag('li',
                            get_string("status", "block_assignments_overview") .
                            get_string("new", "block_assignments_overview"),
                            array('role' => 'treeitem', 'tabindex' => '-1'));
                        $duedate = html_writer::tag('li',
                            get_string("duedate", "block_assignments_overview") . '<br>' .
                            userdate($assignment->duedate),
                            array('role' => 'treeitem', 'tabindex' => '-1'));
                        $time = time();
                        if (($assignment->duedate - $time) <= 0) {
                            $due = get_string('assignmentisdue',
                                'block_assignments_overview');
                        } else {
                            $due = format_time($assignment->duedate - $time);
                        }
                        $timeremaining = html_writer::tag('li',
                            get_string("timeremaining", "block_assignments_overview") . '<br>'
                            . $due,
                            array('role' => 'treeitem', 'tabindex' => '-1'));
                        $assignmentdetails = html_writer::tag('ul',
                            $submitstatus . $duedate . $timeremaining,
                            array('role' => 'group', 'class' => 'assignment_details_ul'));
                        $assignmentsubmissionstatuscount[2]++;
                    }

                    $href = ($this->get_course_modules($course))[$course->id][$assignment->id]->out();
                    $assignmentlink = html_writer::tag('a',
                        $assignmenticonhtml . $assignment->name,
                        array('href' => $href));
                    ...
                    $assignmentlistforacourse .= html_writer::tag('li',
                        $assignmentlink . $assignmentstatusiconhtml . $assignmentdetails,
            ...
            $content .= html_writer::start_tag('li',
                array('class' => 'tree-parent', 'role' => 'treeitem', 'aria-expanded' => 'false', 'tabindex' => '-1'));
            $content .= html_writer::tag('a',
                $course->fullname,
                array('href' => $CFG->wwwroot . '/course/view.php?id=' . $course->id));

            if ($assignmentsubmissionstatuscount[0] !== 0 &&
                $assignmentsubmissionstatuscount[1] === 0 &&
                $assignmentsubmissionstatuscount[2] === 0) {
                $content .= $gradedcourseiconhtml;
            } else if ($assignmentsubmissionstatuscount[0] === 0 &&
                $assignmentsubmissionstatuscount[1] !== 0 &&
                $assignmentsubmissionstatuscount[2] === 0) {
            ...
            $content .= html_writer::start_tag('ul',
                array('role' => 'group', 'class' => 'assignments_ul'));
            $content .= html_writer::tag('p',
                '[T:' . array_sum($assignmentsubmissionstatuscount) . ',G:' .
                $assignmentsubmissionstatuscount[0] . ',S:' . $assignmentsubmissionstatuscount[1] . ',N:' .
                $assignmentsubmissionstatuscount[2] . ']',
                array('class' => 'assignments_quick_count'));
    ...
    /**
     * Produces icons for display in the tree
     *
     * @return array
     */
    public function get_icons() {
        $assignmenticon = new pix_icon('icon',
            get_string('assignment', 'block_assignments_overview'),
            'mod_assignment');
        $assignmenticonhtml = $this->output->render($assignmenticon);

        $gradedassignmenticon = new pix_icon('greensquare',
            get_string('graded', 'block_assignments_overview'),
            'block_assignments_overview', array('class' => 'assignment_status_img'));
        $gradedassignmenticonhtml = $this->output->render($gradedassignmenticon);

        $notsubmittedassignmenticon = new pix_icon('redsquare',
            get_string('new', 'block_assignments_overview'),
            'block_assignments_overview', array('class' => 'assignment_status_img'));
        $notsubmittedassignmenticonhtml = $this->output->render($notsubmittedassignmenticon);

        $notgradedassignmenticon = new pix_icon('yellowsquare',
            get_string('submitted', 'block_assignments_overview'),
            'block_assignments_overview', array('class' => 'assignment_status_img'));
        $notgradedassignmenticonhtml = $this->output->render($notgradedassignmenticon);

        $gradedcourseicon = new pix_icon('greentick',
            get_string('graded', 'block_assignments_overview'),
            'block_assignments_overview', array('class' => 'course_status_img'));
        $gradedcourseiconhtml = $this->output->render($gradedcourseicon);

        $notgradedcourseicon = new pix_icon('yellowtick', get_string('submitted',
            'block_assignments_overview'), 'block_assignments_overview',
            array('class' => 'course_status_img'));
        $notgradedcourseiconhtml = $this->output->render($notgradedcourseicon);

        $notsubmittedcourseicon = new pix_icon('redexclamation',
            get_string('new', 'block_assignments_overview'),
            'block_assignments_overview', array('class' => 'course_status_img'));
